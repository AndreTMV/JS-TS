function largestProductInAGrid(grid:number[][]):number {
  let largestProduct:number = 0;
  let maxOffSet:number = 3;
  for (let row:number = 0; row < grid.length; row++) {
    for (let column:number = 0; column < grid[0].length; column++) {
      let cell:number = grid[row][column];
      let product:number = 0;
      // Vertical
      product = cell;
      for (let i:number = 1; i <= maxOffSet; i++) {
        if (grid[row] && grid[row][column + i]) {
      ...
      product = cell;
      for (let i:number = 1; i <= maxOffSet; i++) {
        if (grid[row + i] && grid[row + i][column]) {
      ...
      product = cell;
      for (let i:number = 1; i <= maxOffSet; i++) {
        if (grid[row + i] && grid[row + i][column + i]) {
...
console.log(largestProductInAGrid(grid));
